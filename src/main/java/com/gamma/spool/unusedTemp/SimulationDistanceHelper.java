...
public class SimulationDistanceHelper {
    ...
    public void tickStart() {
        ...
        for (ChunkCoordIntPair key : ForgeChunkManager.getPersistentChunksFor(world)
            .keys()) {
    ...
    public void addTick(NextTickListEntry entry) {
        synchronized (pendingTickCandidates) {
            pendingTickCandidates.add(entry);
        }
        long key = ChunkCoordIntPair.chunkXZ2Int(entry.xCoord >> 4, entry.zCoord >> 4);
        HashSet<NextTickListEntry> entries;
        synchronized (chunkTickMap) {
            entries = chunkTickMap.get(key);
            if (entries == null) {
                entries = new HashSet<>();
    ...
    public void setServerVariables(Set<NextTickListEntry> pendingTickListEntriesHashSet,
        BiPredicate<Integer, Integer> chunkExists) {
        this.pendingTickListEntriesHashSet = pendingTickListEntriesHashSet; // This will just be `spool$pendingTickList`
