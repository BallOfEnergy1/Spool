...
public class ConcurrentExtendedBlockStorageWrapper extends ConcurrentExtendedBlockStorage implements SubChunkBlockHook {
    ...
    private AtomicReference<NibbleArray> b2High;
    private AtomicReference<byte[]> b3;
    private AtomicReference<NibbleArray> m1High;
    private AtomicReference<byte[]> m2;
    ...
    public NibbleArray eid$getB2Low() {
        if (blockMSBArray == null) return null;
    ...
    public void eid$setB2Low(NibbleArray data) {
        if (blockMSBArray == null) {
            if (data == null) return;
            blockMSBArray = new AtomicReference<>(data);
        } else {
            if (data == null) blockMSBArray = null;
            else blockMSBArray.set(data);
        }
    }

    @Override
    public NibbleArray eid$createB2Low() {
        NibbleArray arr = new NibbleArray(blocksPerSubChunk, 4);
        blockMSBArray = new AtomicReference<>(arr);
        return arr;
    }

    @Override
    public NibbleArray eid$getB2High() {
        if (b2High == null) return null;
        return b2High.get();
    }
    ...
    public void eid$setB2High(NibbleArray data) {
        if (b2High == null) {
            if (data == null) return;
            b2High = new AtomicReference<>(data);
        } else {
            if (data == null) b2High = null;
            else b2High.set(data);
        }
    }
    ...
    public NibbleArray eid$createB2High() {
        NibbleArray arr = new NibbleArray(blocksPerSubChunk, 4);
        b2High = new AtomicReference<>(arr);
        return arr;
    }
    ...
    public void eid$setM1Low(NibbleArray m1Low) {
        blockMetadataArray.set(m1Low);
    }

    @Override
    public NibbleArray eid$getM1High() {
        if (m1High == null) return null;
        return m1High.get();
    }
    ...
    public void eid$setM1High(NibbleArray m1High) {
        if (this.m1High == null) {
            ...
            this.m1High = new AtomicReference<>(m1High);
        } else {
            if (m1High == null) this.m1High = null;
            else this.m1High.set(m1High);
        }
    }
    ...
    public NibbleArray eid$createM1High() {
        NibbleArray arr = new NibbleArray(blocksPerSubChunk, 4);
        m1High = new AtomicReference<>(arr);
        return arr;
    }
    ...
    public void eid$setM2(byte[] m2) {
        if (this.m2 == null) {
            if (m2 == null) return;
            this.m2 = new AtomicReference<>(m2);
        } else {
